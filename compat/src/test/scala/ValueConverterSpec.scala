...
final class ValueConverterSpec
  ...
  "Scalar value converters" should {
    ...
    "support maxKey" >> {
      "from JSON" in {
        implicitly[BSONValue](ValueConverters.JsMaxKey) must_=== BSONMaxKey
      }

      "to BSON" in {
        implicitly[JsObject](BSONMaxKey) must_=== ValueConverters.JsMaxKey
      }
    }

    "support minKey" >> {
      "from JSON" in {
        implicitly[BSONValue](ValueConverters.JsMinKey) must_=== BSONMinKey
      }

      "to BSON" in {
        implicitly[JsObject](BSONMinKey) must_=== ValueConverters.JsMinKey
      }
    }
    ...
    "support undefined" >> {
      "from JSON" in {
        implicitly[BSONValue](
          ValueConverters.JsUndefined) must_=== BSONUndefined
      }

      "to BSON" in {
        implicitly[JsObject](BSONUndefined) must_=== ValueConverters.JsUndefined
      }
    }
  }
